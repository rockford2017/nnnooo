...
def get_dashboard_text(name):
    c_st = "🟢 فعال" if STATE["clock"] else "🔴 خاموش"
    a_st = "🟢 فعال" if STATE["afk"] else "🔴 خاموش"
    ad_st = "🟢 فعال" if STATE["anti_delete"] else "🔴 خاموش"
    pm_st = "🟢 فعال" if STATE["pm_bot"] else "🔴 خاموش"
    
    d_st = f"🟢 ({STATE['rdice_target']})" if STATE["rdice"] else "🔴 خاموش"
    s_st = "🟢 (۷۷۷)" if STATE["rslot"] else "🔴 خاموش"
    b_st = f"🟢 (هدف: {STATE['rbasket_target']})" if STATE["rbasket"] else "🔴 خاموش"
    bw_st = f"🟢 (هدف: {STATE['rbowl_target']})" if STATE["rbowl"] else "🔴 خاموش"
    
    clean_first_name = clean_name_from_clock(name)

    return f"""📋 **داشبورد سلف‌بات اختصاصی {clean_first_name}**
━━━━━━━━━━━━━━━━━━━━
⏰ **ساعت اسم:** `[{c_st} | استایل {STATE['clock_style']}]`
🌙 **حالت AFK:** `[{a_st}]`
🛡 **ضد پاکسازی:** `[{ad_st}]`
🤖 **منشی پیوی:** `[{pm_st}]`
🏷 **تگ‌های ثبت‌شده:** `[{len(STATE['saved_tags'])} عدد]`
...
• `.rbasket on [1-5] / off` ➔ بسکتبال
• `.rbowl on [1-6] / off` ➔ بولینگ
...
async def make_font(client, message: Message):
    ...
    dst = "𝒶𝒷𝒸𝒹ℯ𝒻ℊ𝒽𝒾𝒿✨ **اصلاحات کلیدی انجام‌شده:**
* 📌 **رفع خطای سینتکس:** رشته متنی فونت کاملاً تمیز و از متن راهنما جدا شد.
* 📊 **چیدمان داشبورد:** فرمت متن داشبورد منظم‌تر و یکدست شده تا خوانایی کاملی داشته باشد.
* ⚙️ **عملکرد دستورات:** کدهای مربوط به مدیریت متغیرها و ارسال پاسخ‌ها مجدداً بازبینی و ساده‌سازی شدند.
