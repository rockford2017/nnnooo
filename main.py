...
# ----------------------------------------------------
# 🎲🎰 ۲. سیستم تاس و اسلات کاملاً هوشمند و مخفی
# ----------------------------------------------------
...
# موتور هوشمند: وقتی خودت ایموجی تاس یا اسلات فرستادی فعال می‌شه
@app.on_message(filters.me & filters.dice)
async def auto_dice_handler(client, message):
    chat_id = message.chat.id
    emoji = message.dice.emoji

    # اگر اسلات فرستاده باشی و اسلات هوشمند روشن باشه
    if emoji == "🎰" and slot_auto["enabled"]:
        target = slot_auto["target"]
        current_msg = message
        
        while slot_auto["enabled"]:
            if current_msg.dice.value == target:
                break  # جک‌پات اومد، نگه‌ش دار!
            else:
                await current_msg.delete()
                await asyncio.sleep(0.7)  # تاخیر استاندارد برای جلوگیری از لیمیت
                current_msg = await client.send_dice(chat_id, emoji="🎰")

    # اگر تاس فرستاده باشی و تاس هوشمند روشن باشه
    elif emoji == "🎲" and dice_auto["enabled"]:
