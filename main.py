...
# ----------------------------------------------------
# 🎲🎰 ۲. سیستم تاس و اسلات اولترا سریع (سرعت نور ⚡️)
# ----------------------------------------------------
...
async def auto_dice_handler(client, message):
    ...
    if emoji == "🎰" and slot_auto["enabled"]:
        ...
        while slot_auto["enabled"]:
            if current_msg.dice.value == target:
                break  # جک‌پات (۷۷۷) آمد، پیام می‌ماند!
            else:
                old_msg = current_msg
                # ابتدا سریعاً پیام جدید ارسال می‌شود
                current_msg = await client.send_dice(chat_id, emoji="🎰")
                # سپس بدون هیچ معطلی پیام قبلی حذف می‌گردد
                try:
    ...
    # موتور تاس لحظه‌ای
        ...
        while dice_auto["enabled"]:
            ...
            if is_match:
                break  # عدد هدف آمد، پیام می‌ماند!
            else:
                old_msg = current_msg
                current_msg = await client.send_dice(chat_id, emoji="🎲")
                try:
