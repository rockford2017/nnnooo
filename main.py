...
• `.rbasket on [1-5]` ➔ روشن کردن بسکتبال (هدف: ۵)
• `.rbasket off` ➔ خاموش کردن بسکتبال
• `.rbowl on [1-6]` ➔ روشن کردن بولینگ (هدف: ۶)
...
async def make_font(client, message: Message):
    if len(message.command) < 2:
        ...
        return
    text = " ".join(message.command[1:])
    f1 = text.translate(str.maketrans("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ", "𝒶𝒷𝒸𝒹ℯ𝒻ℊ𝒽𝒾𝒿```python
