...
# ----------------------------------------------------
# 🔄 ۹. ارسال تکراری با زمان مشخص (ثانیه) و زمان‌بندی
# ----------------------------------------------------
async def send_loop_msg(chat_id, text):
    try:
...
async def loop_message(client, message):
    try:
        ...
        # ثبت زمان‌بندی دقیق بر حسب ثانیه
        scheduler.add_job(
            send_loop_msg,
            'interval',
            seconds=seconds,
        ...
        # ارسال بار اول بلافاصله
        await send_loop_msg(chat_id, text_to_send)
        ...
        await message.edit_text(
            "❌ **فرمت اشتباه!**\n"
            "نحوه استفاده (زمان بر حسب ثانیه):\n"
            "`.loop -100123456789 260 متن پیام`\n\n"
            "*(مثال بالا یعنی دقیقاً هر ۴ دقیقه و ۲۰ ثانیه پیام ارسال کن)*"
        )
