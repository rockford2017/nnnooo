...
if __name__ == "__main__":
    print("🚀 Starting Selfbot natively with app.run()...")
    
    # تنظیمات اولیه موقع استارت استاندارد
    async def init_bot():
        me = await app.get_me()
    ...
    # ران کردن اکستنشن‌های اولیه
    app.loop.run_until_complete(app.start())
    app.loop.run_until_complete(init_bot())
    
    print("🚀 Selfbot is ONLINE & Listening for commands!")
    
    # ماندن در حالت شنود کامل پیام‌ها
    app.run()
