...
def get_dashboard_text():
    ...
    afk_status = f"🟢 فعال ({STATE['afk_reason']})" if STATE["afk"] else "🔴 خاموش"
...
🔄 **حلقه ارسال:**
• `.loop [آیدی/چت] [ثانیه] [متن]` ➔ ارسال تکراری
• `.loops` ➔ مشاهده ارسال‌های فعال
• `.stoploop` ➔ توقف تمام ارسال‌ها

📌 **منشن‌ها و پیام‌ها:**
• `.tags` ➔ ارسال منشن‌ها به پیام‌های ذخیره‌شده
• `.cleartags` ➔ پاکسازی لیست منشن‌ها
• `.del [تعداد]` ➔ پاکسازی پیام‌های شما
• `.purge` ➔ پاک کردن پیام‌ها (روی پیام شروع ریپلای کنید)
• `.calc [عبارت]` ➔ ماشین حساب
• `.ping` ➔ بررسی پینگ و وضعیت"""
...
async def rslot_command(client, message: Message):
    ...
    if sub == "on":
        ...
        await message.edit_text("🎰 **اسلات خودکار روشن شد!**\n\n💡 *حالا توی گروه یک ایموجی 🎰 بفرست تا خودش برای ۷۷۷ تکرار کنه.*")
    else:
