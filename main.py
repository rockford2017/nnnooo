...
def clean_name_from_clock(name):
    """حذف هرگونه ساعت یا جداکننده قبلی از اسم برای جلوگیری از تکرار ساعت"""
    if not name:
        return "User"
    # حذف بخش‌های مربوط به ساعت مثل | 12:43 یا | ۱۲:۴۳
...
async def toggle_clock(client, message: Message):
    ...
    if not STATE["clock"]:
        # اگر ساعت خاموش شد، اسم را به حالت اولیه برگردان
...
async def start_loop(client, message: Message):
    try:
        ...
        # پاسخ دقیقاً مطابق فرمت درخواستی شما
