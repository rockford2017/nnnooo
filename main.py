...
STATE = {
    ...
    "rbowl": False,
    "rbowl_target": 6,
    "reroll_delay": 0.2, # تاخیر پیش‌فرض برای عدم لو رفتن و جلوگیری از بلاک
    "loops": {},
    "schedules": [],
    "notes": {}
}
...
🏀 **بسکتبال:** [ {basket_st} ]
🎳 **بولینگ:** [ {bowl_st} ]
⏱ **تاخیر پرتاب:** [ `{STATE['reroll_delay']}` ثانیه ]
━━━━━━━━━━━━━━━━━━━━
...
• `.rdelay [ثانیه]` ➔ تنظیم سرعت پرتاب مجدد (مثلا `.rdelay 0.2`)
...
# --- HIGH-SPEED AUTOMATIC REROLL HANDLER (STEALTH MODE) ---
...
async def handle_dice_reroll(client, message: Message):
    ...
    if STATE["rdice"] and emoji == "🎲":
        ...
        if val == 64: matched = True # 64 یعنی 777
    ...
    # اگر نتیجه با هدف یکی نبود:
    if not matched and any([STATE["rdice"] and emoji == "🎲",
                             STATE["rslot"] and emoji == "🎰",
                             STATE["rbasket"] and emoji == "🏀",
                             STATE["rbowl"] and emoji == "🎳"]):
        try:
            # ارسال پرتاب جدید
            await client.send_dice(chat_id, emoji=emoji)
            # تاخیر کوتاه‌مدت قبل از پاک‌کردن قبلی برای جلوگیری از لیمیت تلگرام
            await asyncio.sleep(STATE["reroll_delay"])
            # پاک کردن پیام پوچ قبلی
...
async def make_font(client, message: Message):
    ...
    dst = "𝒶𝒷𝒸𝒹ℯ𝒻ℊ𝒽𝒾𝒿`𝒷𝒸𝒹ℯ𝒻ℊ𝒽𝒾𝒿𝓀𝓁𝓂𝓃ℴ𝓅𝓆𝓇𝓈𝓉𝓊𝓋𝓌𝓍𝓎𝓏𝒜ℬ𝒞𝒟ℰℱ𝒢ℋℐ𝒥𝒦ℒℳ𝒩𝒪𝒫𝒬ℛ𝒮𝒯𝒰𝒱𝒲𝒳𝒴𝒵"
