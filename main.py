...
# ----------------- GAME ENGINE (DICE & SLOT HANDLERS) -----------------
...
async def process_dice_slot(client, message: Message):
    # پردازش تاس 🎲
    if message.dice.emoji == "🎲" and STATE["dice_active"]:
        ...
        if hit:
            print(f"🎯 تاس به هدف رسید: {val}")

    # پردازش اسلات 🎰
    elif message.dice.emoji == "🎰" and STATE["slot_active"]:
        val = message.dice.value
        if val == STATE["slot_target"]:  # 64 یعنی جک‌پات (777)
            print(f"🎉 اسلات برنده شد: {val}")
