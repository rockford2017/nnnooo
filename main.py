...
async def toggle_antidel(client, message):
    ...
    if len(cmd) > 1:
        if cmd[1].lower() == "on":
            CONFIG["IS_ANTIDEL_ON"] = True
            await message.edit_text("🟢 **ضد پاکسازی فعال شد.**")
        elif cmd[1].lower() == "off":
            CONFIG["IS_ANTIDEL_ON"] = False
            await message.edit_text("🔴 **ضد پاکسازی غیرفعال شد.**")
        await save_config_to_telegram()
...
async def toggle_antiedit(client, message):
    ...
    if len(cmd) > 1:
        if cmd[1].lower() == "on":
            CONFIG["IS_ANTIEDIT_ON"] = True
            await message.edit_text("🟢 **ضد ویرایش فعال شد.**")
        elif cmd[1].lower() == "off":
            CONFIG["IS_ANTIEDIT_ON"] = False
            await message.edit_text("🔴 **ضد ویرایش غیرفعال شد.**")
        await save_config_to_telegram()
