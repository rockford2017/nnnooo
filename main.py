...
loop_running = False  # وضعیت ارسال تکراری
loop_task = None     # برای مدیریت اجرای پس‌زمینه Loop
...
• `.loop [آیدی چت] [ثانیه] [متن]` ➔ ارسال تکراری زمان‌بندی‌شده
• `.unloop` یا `.stoploop` ➔ توقف ارسال تکراری
...
# ----------------------------------------------------
# 💣 ۴. ارسال پیام تکراری (Spam & Custom Loop)
# ----------------------------------------------------
...
async def custom_loop_messages(client, message):
    ...
    try:
        ...
        if loop_running and loop_task:
            loop_running = False
            loop_task.cancel()

        loop_running = True
        loop_task = asyncio.create_task(loop_worker(client, target_chat, delay, text))
...
async def stop_loop(client, message):
    ...
    if loop_running:
        ...
        if loop_task:
            loop_task.cancel()
        await message.edit_text("🛑 **ارسال تکراری (Loop) متوقف شد.**")
    else:
