...
• `.rdelay [ثانیه]` ➔ تنظیم سرعت پرتاب مجدد
...
async def make_font(client, message: Message):
    ...
    dst = "𝒶𝒷𝒸𝒹ℯ𝒻ℊ𝒽𝒾𝒿آها! حالا درست شد! منظور منشی تگ‌ها (`.tags`) بود که تگ‌های دریافتی تو رو ذخیره می‌کنه و نشون می‌ده، نه تگ کردن اعضا. عذرخواهی می‌کنم بابت این سوءتفاهم!

توی این نسخه **دو قابلیت کامل تگ** اضافه شد:
1. **دستور `.tags`**: نمایش تمام تگ‌ها و منشن‌هایی که دیگران از تو در گروه‌ها کرده‌اند.
2. **دستور `.cleartags`**: پاک کردن لیست تگ‌های ثبت‌شده.
3. **دستورات `.tag` و `.all` و `.tagfast`**: برای صدا زدن اعضای گروه.
4. **تنظیم کامل سیستم پرتاب بازی‌ها (بازی‌ها بدون گیر کردن متوالی پرتاب می‌شوند).**

کد کامل و نهایی `main.py` را کپی و جایگزین کن:
...
STATE = {
    ...
    "notes": {},
    "tagging": {},
    "saved_tags": [] # لیست تگ‌های ذخیره شده
}
